	...
	for (i = 0; i < TOUCH_PWR_NUM; ++i) {
		if (ts_pwr[i].type == 1 && gpio_is_valid(ts_pwr[i].value)) {
			...
			if (!strncmp(ts_pwr[i].name, "low", strlen("low")))
				ret =
				    gpio_request_one(ts_pwr[i].value, GPIOF_OUT_INIT_LOW,
						     gpio_request_name);
			else
				ret =
				    gpio_request_one(ts_pwr[i].value, GPIOF_OUT_INIT_HIGH,
						     gpio_request_name);
			...
			if (regulator_count_voltages(ts->vdd_regulator[i]) > 0) {
				ret =
				    regulator_set_voltage(ts->vdd_regulator[i], ts_pwr[i].value,
							  ts_pwr[i].value);
				if (ret) {
					TOUCH_ERR_MSG
					    ("Error(ret=%d) set regulator(%s) voltage %d\n", ret,
		...
		if (tmp[0] & MIT_TOUCH_KEY_EVENT) {
			...
			if (unlikely(touch_debug_mask_ & DEBUG_GET_DATA))
				TOUCH_INFO_MSG("key_code=[0x%02x-%d], state=[%d]\n",
					       ...
					       data->curr_button.key_code, data->curr_button.state);
		} else {
			...
			if (unlikely(touch_debug_mask_ & DEBUG_GET_DATA)) {
				TOUCH_INFO_MSG
				    ("<%d> pos(%4d,%4d) w_m[%2d] w_n[%2d] w_o[%2d] p[%2d] s[%d]\n",
				     id, data->curr_data[id].x_position,
				     data->curr_data[id].y_position,
				     data->curr_data[id].width_major,
				     data->curr_data[id].width_minor,
				     data->curr_data[id].width_orientation,
				     data->curr_data[id].pressure, state);
			}
			...
			if (data->curr_data[id].status == FINGER_PRESSED
				...
				if (likely(touch_debug_mask_ & DEBUG_ABS_POINT)) {
					if (lockscreen_stat == 1) {
						TOUCH_INFO_MSG
						    ("%d finger pressed : <%d> x[XXX] y[XXX] z[XXX]\n",
						     data->touch_count_num, id);
					} else {
						TOUCH_INFO_MSG
						    ("%d finger pressed : <%d> x[%3d] y[%3d] z[%3d]\n",
						     data->touch_count_num, id,
				...
				if (likely(touch_debug_mask_ & DEBUG_ABS_POINT)) {
					if (lockscreen_stat == 1) {
						TOUCH_INFO_MSG
						    ("touch_release[%s] : <%d> x[XXX] y[XXX] M:XX\n",
						     data->palm ? "Palm" : " ", id);
					} else {
						TOUCH_INFO_MSG
						    ("touch_release[%s] : <%d> x[%3d] y[%3d] M:%d\n",
						     data->palm ? "Palm" : " ", id,
...
static int mms_get_data(struct i2c_client *client, struct touch_data *data)
{
	...
	if (event_type >= 0x1 && event_type <= 0xa) {
		...
		if (ts->buf[1] == MIT_REQUEST_THERMAL_INFO) {
			...
			dummy_buf = kzalloc(PAGE_SIZE, GFP_KERNEL);
			if (dummy_buf != NULL) {
...
static int mms_power(struct i2c_client *client, int power_ctrl)
{
	...
	if (atomic_read(&ts->pdata->curr_pwr_state) == power_ctrl) {
		...
		do {
			if (ts_pwr[i].type == 1) {
				...
				if (ts->vdd_regulator[i] != NULL && !IS_ERR(ts->vdd_regulator[i])) {
					...
					if (ret) {
						TOUCH_INFO_MSG
						    ("power[%d]: regulator enable failed ret =%d\n",
						     i, ret);
					} else {
	...
	atomic_set(&ts->pdata->curr_pwr_state, power_ctrl);
...
static int mms_firmware_img_parse_show(const char *image_bin, char *show_buf, int ret)
{
	...
	ret +=
	    sprintf(show_buf + ret, "\tcontains_full_binary[%d]\n", fw_hdr->contains_full_binary);
	ret +=
	    sprintf(show_buf + ret, "\tbinary_offset[%d (0x%04x)]\n", fw_hdr->binary_offset,
		    fw_hdr->binary_offset);
...
static int mms_ic_ctrl(struct i2c_client *client, u32 code, u32 value)
{
	...
	case IC_CTRL_FIRMWARE_IMG_SHOW:
		ret =
		    mms_firmware_img_parse_show((const char *)param->v1, (char *)param->v2,
						param->v3);
		break;
...
=======
			...
			if (buf) {
				...
				ret += sprintf(buf + ret, "======================\n");
				ret +=
				    sprintf(buf + ret, "F/W Version : %X.%02X\n",
					    ts->module.version[0], ts->module.version[1]);
				ret +=
				    sprintf(buf + ret, "F/W Product : %s\n",
					    ts->module.product_code);
				ret +=
				    sprintf(buf + ret, "F/W Row : %d, Col : %d\n", ts->dev.row_num,
					    ts->dev.col_num);
				if (ts->module.otp == OTP_NOT_SUPPORTED)
					ret += sprintf(buf + ret, "OTP : F/W Not support\n");
				else {
					ret +=
					    sprintf(buf + ret, "OTP : %s\n",
						    (ts->module.otp ==
						     OTP_APPLIED) ? "Applied" : "None");
				}
				ret += sprintf(buf + ret, "======================\n");
			}
		}
	...
	case IC_CTRL_TESTMODE_VERSION_SHOW:
		if (ts->module.product_code[0])
			TOUCH_INFO_MSG("F/W : %X.%02X (%s)\n", ts->module.version[0],
				       ts->module.version[1], ts->module.product_code);
		if (param) {
			buf = (char *)param->v1;
			if (buf) {
				ret +=
				    sprintf(buf + ret, "%X.%02X(%s)\n", ts->module.version[0],
					    ts->module.version[1], ts->module.product_code);
			}
		}
		break;
...
static int mms_reg_control_store(struct i2c_client *client, const char *buf)
{
	...
	case 2:
		...
		break;
	default:
		TOUCH_INFO_MSG("usage: echo [1(read)|2(write)], [reg address0], [reg address1]");
		TOUCH_INFO_MSG("[length(read)|value(write)] > reg_control\n");
...
=======
	len =
	    snprintf(buf, PAGE_SIZE - len, "Firmware Version : %X.%02X\n", ts->module.version[0],
		     ts->module.version[1]);
	len += snprintf(buf + len, PAGE_SIZE - len, "FW Product : %s\n", ts->module.product_code);
	len += snprintf(buf + len, PAGE_SIZE - len, "=======RESULT========\n");
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "OPEN /  SHORT Test : %s\n",
		     ts->pdata->selfdiagnostic_state[SD_OPENSHORT] == 1 ? "PASS" : "FAIL");
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "SLOPE Test : %s\n",
		     ts->pdata->selfdiagnostic_state[SD_SLOPE] == 1 ? "PASS" : "FAIL");
...
static ssize_t mms_self_diagnostic_show(struct i2c_client *client, char *buf)
{
	...
	if (ts->module.otp == OTP_APPLIED) {
		...
		ret +=
		    sprintf(buf + ret, "RAW DATA SPEC (UPPER : %d  LOWER : %d  MARGIN : %d)\n",
			    ts->pdata->limit->raw_data_otp_max, ts->pdata->limit->raw_data_otp_min,
			    ts->pdata->limit->raw_data_margin);
		TOUCH_INFO_MSG("RAW DATA SPEC (UPPER : %d  LOWER : %d MARGIN : %d)\n",
			       ts->pdata->limit->raw_data_otp_max,
			       ts->pdata->limit->raw_data_otp_min,
			       ts->pdata->limit->raw_data_margin);
	} else {
	...
	if (ts->pdata->selfdiagnostic_state[SD_RAWDATA] == 0) {
		...
		for (row = 0; row < MAX_ROW; row++) {
			...
			for (col = 0; col < MAX_COL; col++) {
				...
				if (ts->mit_data[row][col] <= limit_upper
				    && ts->mit_data[row][col] >= limit_lower) {
					ret += sprintf(buf + ret, " ,");
				} else {
...
=======
	...
	memset(buf, 0, PAGE_SIZE);
	len =
	    snprintf(buf, PAGE_SIZE, "Firmware Version : %X.%02X\n", ts->module.version[0],
		     ts->module.version[1]);
	len += snprintf(buf + len, PAGE_SIZE - len, "FW Product : %s\n", ts->module.product_code);
	if (ts->module.otp == OTP_NOT_SUPPORTED) {
		...
		len +=
		    snprintf(buf + len, PAGE_SIZE - len, "OTP : %s\n",
			     (ts->module.otp == OTP_APPLIED) ? "Applied" : "None");
	}
	len += snprintf(buf + len, PAGE_SIZE - len, "=====================\n");
	if (ts->pdata->check_openshort)
		len +=
		    snprintf(buf + len, PAGE_SIZE - len, "OpenShort : %5d , %5d\n", ts->o_max,
			     ts->o_min);
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "Slope     : %5d , %5d\n", ts->s_max, ts->s_min);
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "Rawdata   : %5d , %5d\n", ts->r_max, ts->r_min);
	len += snprintf(buf + len, PAGE_SIZE - len, "=======RESULT========\n");
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "Channel Status : %s\n",
		     (ts->pdata->selfdiagnostic_state[SD_OPENSHORT] *
		      ts->pdata->selfdiagnostic_state[SD_SLOPE]) == 1 ? "PASS" : "FAIL");
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "Raw Data : %s\n",
		     ts->pdata->selfdiagnostic_state[SD_RAWDATA] == 1 ? "PASS" : "FAIL");
	write_file(sd_path, buf, 0);
	return len;
}
...
static ssize_t mms_lpwg_show(struct i2c_client *client, char *buf)
{
	...
	len +=
	    snprintf(buf + len, PAGE_SIZE - len,
		     "LPWG Mode : %X (0:None, 1:Knock-On, 10, Knock-Code)\n",
		     ts->pdata->lpwg_mode);
...
static ssize_t mms_lpwg_store(struct i2c_client *client, char *buf1, const char *buf2)
{
	...
	if (buf1 != NULL && !strcmp(buf1, "area")) {
		...
		TOUCH_DEBUG_MSG("Active Area - X1:%d, X2:%d, Y1:%d, Y2:%d\n",
				ts->pdata->active_area_x1, ts->pdata->active_area_x2,
				ts->pdata->active_area_y1, ts->pdata->active_area_y2);
		return 0;
	}

	/* set double tap check */
	if (buf1 != NULL && !strcmp(buf1, "tap_check")) {
		ret = sscanf(buf2, "%d", &tap_check);
		if (ret > 0)
