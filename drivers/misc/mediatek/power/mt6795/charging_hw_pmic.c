...
{
    battery_log(BAT_LOG_CRTI,"[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x,[0x%x]=0x%x\n",
...
int upmu_is_chr_det_hal(void)
{
    ...
    else
    {
        if( mt_usb_is_device() )
        {
        	battery_log(BAT_LOG_CRTI, "[upmu_is_chr_det_hal] Charger exist and USB is not host\n");
            return 1;
        }
        else
        {
            battery_log(BAT_LOG_CRTI, "[upmu_is_chr_det_hal] Charger exist but USB is host\n");
            return 0;
        }
    }
}
...
// chr_plug_out_sw_detect
    ...
    if(upmu_is_chr_det_hal()==1) //sync USB device/otg state
    {
        ...
        else
        {
            ...
            battery_log(BAT_LOG_CRTI,"[chr_plug_out_sw_detect] ich_low_val=%d, is_charge_complete=%d\n",
            ...
            if(is_chr_det()==1) // for evb
            {
                ...
                battery_log(BAT_LOG_CRTI,"[chr_plug_out_sw_detect] Reg[0x%x]=0x%x\n", 0x816C, upmu_get_reg_value(0x816C) );
                //
                msleep(10);
                if(is_chr_det()==1)
                {
...
static kal_uint32 charging_get_battery_status(void *data)
{
    ...
    *(kal_bool*)(data) = 0; // battery exist
    battery_log(BAT_LOG_CRTI,"[charging_get_battery_status] EVB no HW battery\n");
    #endif
...
static kal_uint32 charging_get_charger_det_status(void *data)
{
     ...
     *(kal_bool*)(data) = 1;
     battery_log(BAT_LOG_CRTI,"[charging_get_charger_det_status] no HW function\n");
#endif
...
static kal_uint32 charging_get_is_pcm_timer_trigger(void *data)
{
 ...
 static kal_uint32 charging_set_platform_reset(void *data)
 {
     ...
     arch_reset(0,NULL);
     #else
     battery_log(BAT_LOG_CRTI, "wait arch_reset ready\n");
     #endif
#endif
